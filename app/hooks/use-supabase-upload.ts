...
const useSupabaseUpload = (options: UseSupabaseUploadOptions) => {
  ...
  const onUpload = useCallback(async () => {
    ...
    const responses = await Promise.all(
      filesToUpload.map(async (file) => {
        try {
          ...
          const signedUrlResponse = await fetch("/upload", {
            method: "POST",
            body: formData,
          })
          ...
          if (!signedUrlResponse.ok) {
            ...
            return { name: file.name, message: errorText }
          }

          const responseData = (await signedUrlResponse.json()) as {
            signedUrl: string
            path: string
          }
    ...
    setLoading(false)
  }, [files, path, bucketName, errors, successes, cacheControl])
